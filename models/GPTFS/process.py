...
# Consider the following context: phonemes are defined by their relationship to words : changing a phoneme changes the word . learning a phoneme inventory with little supervision has been a longstanding challenge with important applications to under - resourced speech technology . 
# In that context, which task could you use a neural discrete representation learning model for, and why? 
# 
...
out_name = 'local_dataset/test.json'
in_name = '../T5/local_context_dataset/test.json'
with open(in_name, 'r') as file_j:
    wf = open(out_name, 'w')
    for idxn, line in tqdm(enumerate(file_j), "Encoding"):
        pdf_dict = json.loads(line)
