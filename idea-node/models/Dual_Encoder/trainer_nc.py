...
from model_nc import ContrastiveDualEncoder
...
class Trainer:
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, tokenizer=None, sampler=None, train=True):
        ...
        if 'roberta' in self.args.model.lower():
            self.keys = ['hr_token_ids', 'hr_mask', 'tail_token_ids', 'tail_mask', 'head_token_ids', 'head_mask']
        else:
            self.keys = ['hr_token_ids', 'hr_mask', 'tail_token_ids', 'tail_mask', 'head_token_ids', 'head_mask', 'hr_token_type_ids']
